defmodule Paths do
  defmodule Path do
    defstruct nodes: MapSet.new(), path: [], last: nil
  end
  ...
  defp visitable(node, path) do
    String.upcase(node) == node or not MapSet.member?(path.nodes, node)
  end
end

IO.puts(Paths.run("sample.txt") == 10)
IO.puts(Paths.run("sample2.txt") == 19)
IO.puts(Paths.run("sample3.txt") == 226)
IO.puts(Paths.run("input.txt") == 4104)
